...
with torch.no_grad():
    ...
    for i in range(n_steps):
        noisy_map = m.sample((pop_size,model.num_params())).cuda()
        actions = []
        ...
        print('%i %f' % (i, rewards.mean()))

        if done.sum() > 0:
            obs = env.reset()
            print('reset')

        diff_reward = rewards[:pop_size] - rewards[pop_size:]
        topk = diff_reward.abs().topk(topk_size, largest = True)[1]
        iterate = (noisy_map * diff_reward.unsqueeze(1))[topk]
        model.add_params(iterate.mean(dim=0) / rewards.std())